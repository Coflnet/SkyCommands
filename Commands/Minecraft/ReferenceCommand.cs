...
namespace Coflnet.Sky.Commands.MC
{
    public class ReferenceCommand : McCommand
    {
        ...
        private async Task SniperReference(MinecraftSocket socket, string uuid, FlipInstance flip, string algo)
        {
            var reference = await AuctionService.Instance.GetAuctionAsync(flip.Context["reference"]);
            Console.WriteLine(JSON.Stringify(flip.Context));
